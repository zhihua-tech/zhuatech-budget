...
新增期间、来源余额、职责分离、双方责任中心、财务审批和年度计划影响门禁，详见 [预算调拨治理](docs/ENTERPRISE_BUDGET_TRANSFER.md)。
        ...
        ZhuaTech Budget 是知华科技（上海如静知华信息科技有限公司）发布的前后端分离企业应用社区源码版，面向年度预算、滚动预测、预算调整、执行控制和偏差分析管理。官网：[https://www.zhuatech.cn/](https://www.zhuatech.cn/)。

        ## 企业版 V2.0

        在原有四大业务模块基础上，新增企业控制中心：支持组织与账期维度、幂等防重、经办/管理员职责分离、审批闭环、附件 SHA-256 元数据、办结凭证门槛、外部适配器回执、乐观锁和全程审计；并实现预算可用额、逐笔占用、超预算拦截与授权覆盖标记。详见[企业版能力说明](docs/ENTERPRISE.md)。
        ...
        | 模块 | 已实现能力 |
        | --- | --- |
        | 预算编制 | 按组织、科目和期间编制年度预算 |
| 预算调整 | 申请追加、调剂、冻结和释放预算 |
| 执行控制 | 占用、消耗和超预算事项在线校验 |
| 滚动预测 | 结合实际执行更新全年预测 |
        | 运营总览 | 状态结构、模块负荷、金额指标、风险关注和最近业务 |
        | 领域计算 | 预算可用额度控制，提供可解释计算结果和处理建议 |
| 操作审计 | 创建、修改、删除、流程动作和设置变更均保留操作人及时间 |
        ...
        浏览器打开 `http://localhost:8105`。演示账号：`admin / admin123`、`operator / operator123`。默认密码只能用于本地演示，上线前必须修改。
        ...
        SEO 关键词：全面预算管理、预算编制、预算控制、滚动预测、预算分析、知华科技、上海软件开发、企业信息化、软件项目外包。
